...
public class LinuxAudioRecorder : IAudioRecorder
{
    #region Events
    ...
    #region Injects
    protected readonly ILogger<LinuxAudioRecorder> Logger;
    ...
    public LinuxAudioRecorder(ILogger<LinuxAudioRecorder> logger, ClientSettings settings, IAudioDeviceService audioDeviceService)
    {
        Logger = logger;
        Settings = settings;
    ...
    public async Task<bool> StartAsync()
    {
        ...
        var deviceSelection = inputDevice == null ? "" : $" -D \"{inputDevice}\"";

        RecordProcess = new Process();
        RecordProcess.StartInfo.FileName = "arecord";
        RecordProcess.StartInfo.Arguments = $"-q -t raw -f S16_LE -r {SampleRate} -c 1{deviceSelection}";
